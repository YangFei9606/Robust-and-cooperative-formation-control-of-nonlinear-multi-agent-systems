...
%% Interaction period
    for i = 0:simu_len/step
            ...
            for j = 1 : num_rob
                if read_flag(j) == 0  % No need to read any information for agent j
                    ...
                    if i == 0   % The first time
                        [pos_return(j,1),pos_rob] = vrep.simxGetObjectPosition(clientID, rover_handle(j),-1,vrep.simx_opmode_streaming);
                        [pos_return(j,2),ori_rob] = vrep.simxGetObjectOrientation(clientID, rover_handle(j),-1,vrep.simx_opmode_streaming);
                    else        % Later on
                        [pos_return(j,1),pos_rob] = vrep.simxGetObjectPosition(clientID, rover_handle(j),-1,vrep.simx_opmode_buffer );
                        [pos_return(j,2),ori_rob] = vrep.simxGetObjectOrientation(clientID, rover_handle(j),-1,vrep.simx_opmode_buffer );
                    end
                    ...
                    if now_pos(3)>pi
                        now_pos(3) = now_pos(3) - 2*pi;
                    elseif now_pos(3)<-pi
                        now_pos(3) = now_pos(3) + 2*pi;
                    end
                    
                    % Compare this with the last set of data
                    % If the data is different from the previously recorded ones
        ...
        fprintf("The data recording for time %f s is completed.\n",i*step);
