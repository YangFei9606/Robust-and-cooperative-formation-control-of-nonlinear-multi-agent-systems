...
void loop() {
  ...
  if (tcp_read_flag == 2)  // Receive the flag of host info feeding (Note: read_flag = 2 is used here)
  {
    ...
    int num_data_to_read = 9;
    read_from_TCP(client_pc, num_data_to_read);
    combine_data_to_pc();
    send_Through_TCP(client_pc, 6, data_to_pc, 3  );        // Note that the communication flag = 3 is used here for agent to PC data transmission

    // Allocate the data received to vectors: agent_pos, pos_ref and vel_ref
    allocate_PC_data(num_data_to_read);
    ...
    ////// Now it is the control process (STEP 2: Real-time controller)

    // STEP 2.1 : Calculate delta_pos
    for (i = 0; i < 3; i++)
    {
      delta_pos[i] = agent_pos[i] - pos_ref[i];
    }

    // STEP 2.2: Communication with other agents
    // Skipped for now, there maybe more steps for the inter-agent communication

    // Test for single rover
    for (i = 0; i < 3; i++)
    {
      e_pos[i] = delta_pos[i];
    }
    ...
    // STEP 2.3: Calculate the nominal control input
    u_nom[0] = vel_ref[0] - k_p * e_pos[0] - w_hat[0];
    u_nom[1] = vel_ref[1] - k_p * e_pos[1] - w_hat[1];
    u_nom[2] = vel_ref[2] - k_p * e_pos[2] - w_hat[2];

    Serial.println("The nominal control input is: ");
    print_a_vector(u_nom, 3);
    
    // STEP 2.4: Calculate the actual control input (in the unit of m/s)
    theta = agent_pos[2];
    u_actual[0] = (double) - const_2_3*sin(theta) * u_nom[0] + const_2_3*cos(theta) * u_nom[1] + const_r * u_nom[2];
    u_actual[1] = (double) - const_2_3*sin(PI/3 - theta) * u_nom[0] - const_2_3*cos(PI/3 - theta) * u_nom[1] + const_r * u_nom[2];
    u_actual[2] = (double) const_2_3*sin(PI/3 + theta) * u_nom[0] - const_2_3*cos(PI/3 + theta) * u_nom[1] + const_r * u_nom[2];

    Serial.println("The actual control input is: ");
    ...
    Serial.println("The speed of the motor is (step/sec): ");
    print_a_vector(motor_speed, 3);
